...
class NumericallyAugmentedBertNet(nn.Module):
    def __init__(self,
        ...
        if answering_abilities is None:
            ...
            self.answering_abilities.append("none")
            # jferguson Done
            self.answering_abilities.append("binary")
        else:
    ...
    def forward(self,  # type: ignore
        ...
        # TODO: jferguson - Replace bert with longformer here
        ...
        if answer_as_passage_spans is not None or answer_as_question_spans is not None or answer_as_add_sub_expressions is not None or answer_as_counts is not None or answer_as_none is not None or answer_as_binary is not None:
            ...
            for answering_ability in self.answering_abilities:
                if answering_ability == "passage_span_extraction":
                  ...
                  clamped_binary_answers = util.replace_masked_values(answer_as_binary, mask, 0)
                  log_likelihood_for_binary = torch.gather(binary_log_probs, 1, clamped_binary_answers.unsqueeze(1)).squeeze(1)
                  log_likelihood_for_binary = util.replace_masked_values(log_likelihood_for_binary, mask, -1e7)
